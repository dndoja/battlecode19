...
export default class Crusader extends RobotController{
    constructor(robot){
        super(robot);
        super.getClosestEnemyCastle();
        this.enemyCastles = [];
        this.enemyCastles.push(super.getClosestEnemyCastle());

        let coords = super.getCoordinatesFromCastle();
        for (let i = 0; i < coords.length; i++){
            this.enemyCastles.push(coords[i]);
        }

        robot.log(this.enemyCastles.length + " castles");
        for (let i = 0; i < this.enemyCastles.length; i++){
           // robot.log("X: " + this.enemyCastles[i].x + " Y: " + this.enemyCastles[i].y)
        }
    ...
    checkIfReachedEnemyCastles(){
        ...
        if (unexploredCastles.length !== this.enemyCastles.length){
            ...
            for (let i = 0; i < this.enemyCastles.length; i++){
                //this.robot.log("changed: " + "X: " + this.enemyCastles[i].x + " Y: " + this.enemyCastles[i].y)
            }
    ...
    moveAccordingToMap(){
        super.setDijkstraMap(this.enemyCastlesMap);
        let delta = super.moveAlongDijkstraMap(2);
        if (delta) {
