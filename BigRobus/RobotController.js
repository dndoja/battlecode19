...
import constants from "./constants.js";
import {getDistanceBetweenPoints,getCoordsFromEightBits, padWithZeros,getMapSymmetryType} from "./utils.js";
...
export default class RobotController {
    ...
    getClosestEnemyCastle(){
        ...
        let symmetry = getMapSymmetryType(this.robot.map);

        for (let i = 0; i < friendlyCastles.length; i++){
            let castleF = friendlyCastles[i];
            let symmetricalX = this.robot.map.length - 1 - castleF.x;
            let symmetricalY = this.robot.map.length - 1 - castleF.y;
            if (symmetry === constants.SYMMETRY_HORIZONTAL) {
                return{x: symmetricalX, y: castleF.y};
            }else {
                return{x: castleF.x, y: symmetricalY};
            }
        }
    }
